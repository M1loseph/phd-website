...
class LimitingService(
  ...
  private fun findOrCreateBucket(
      key: String,
      bucketConfig:
          io.github.m1loseph.phdwebsiteanalyticsserver.services.limiting.BucketConfiguration
  ): RateLimitingResult {
    val bucket =
        proxyManager.builder().build(key) {
...
data class RateLimitingResult(val success: Boolean, val timeToNextPossibleCall: Duration)
